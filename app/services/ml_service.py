...
CANDIDATE_ACTIONS = ["retry", "payment_link", "alt_method"]
...
def _get_attempts_so_far(db, payment_id: str) -> int:
    ...
    return db.query(Action).filter(Action.payment_id == payment_id).count()


def _get_past_rate(db, customer_id: str | None) -> float:
    """
    Looks up the customer's historical recovery rate.
    Falls back to a neutral default if no customer_id or no record exists -
    this is decision context only (spec Section 18), not a separate model.
    """
    if not customer_id:
    ...
    return customer.previous_recoveries / customer.previous_failures
...
def predict_all_actions(db, payment) -> dict:
    """
    Given a Payment row, returns recovery probability for every candidate
    action: {"retry": ..., "payment_link": ..., "alt_method": ...}.
    This is the Day 3 deliverable per spec Section 6.
    """
    cause = diagnose(payment)
    attempts = _get_attempts_so_far(db, payment.payment_id)
    days_since_failure = _get_days_since_failure(payment)
    past_rate = _get_past_rate(db, payment.customer_id)
